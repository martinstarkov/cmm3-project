...
class UserInterface(object):
    ...
    def create_header(self):
        # Create the GUI logo.
        utility.create_image(self.frame, utility.relative_to_absolute(__file__, "gui/logo.png"), 0, 1)
...
class VelocityInputField(ToggleInputField):
    def appear(self):
        ...
        self.entries.append(utility.relative_to_absolute(__file__, self.defaults[1]))
...
class ChemicalSpill(MainMenuButton):
    def press(self):
        ...
        parameters["velocity_field_path"] = utility.relative_to_absolute(__file__, parameters["velocity_field_path"])
        ...
        anim = animation.FuncAnimation(figure, func=self.animate_plot, frames=self.sim.steps, interval=1, repeat=False)
        
        self.ui.label["text"] = self.name + " (animating until t=" +  str(self.sim.time_max) + "s," + \
                                            "dt=" + str(self.sim.dt) + "s) \n" + \
                                            "Permanently marking all locations where concentration has ever been > " + str(self.highlight_threshold)
        
        utility.create_button(self.ui.container, "Reset Plot", 1, 0, self.press, pady=(5, 0), ipady=15, fg="black", bg="pink")
        utility.create_button(self.ui.container, "Back", 2, 0, self.ui.create_menu_buttons, pady=(5, 0), ipady=15, fg="black", bg="pink")
    ...
    def animate_plot(self, step: int):
        ...
        above_threshold = np.logical_or(self.sim.concentrations > self.highlight_threshold, self.highlighted > self.highlight_threshold)
...
class ValidationTasks(MainMenuButton):
    def press(self):
        ...
        parameters["reference_file_path"] = utility.relative_to_absolute(__file__, parameters["reference_file_path"])
        
        self.validation = validation.Validation(parameters)
        
        # Create sub containers for the task buttons.
        utility.set_grid_sizes(self.ui.container, rows=[20, 40, 20, 20, 10], columns=[100])
        input_container = utility.create_frame(self.ui.container, 1, 0)
        utility.set_grid_sizes(input_container, rows=[50, 50], columns=[20, 20, 20, 15, 15, 15])

        utility.create_label(input_container, "logarithmic particle divisions (int) =", 1, 0)
        utility.create_label(input_container, "linear dt divisions (int) =", 2, 0)
        utility.create_label(input_container, "≤ particles (ints) ≤", 1, 4)
        utility.create_label(input_container, "≤ dts (s) (floats) ≤", 2, 4)
        
        self.inputs = []
        self.inputs.append(utility.create_entry(input_container, self.data["rmse"]["particle_divisions"], 1, 1, sticky="EW", padx=(0, 5), width=5))
        self.inputs.append(utility.create_entry(input_container, self.data["rmse"]["particle_min"], 1, 3, sticky="EW", width=5))
        self.inputs.append(utility.create_entry(input_container, self.data["rmse"]["particle_max"], 1, 5, sticky="EW", width=5))
        self.inputs.append(utility.create_entry(input_container, self.data["rmse"]["dt_divisions"], 2, 1, sticky="EW", padx=(0, 5), width=5))
        self.inputs.append(utility.create_entry(input_container, self.data["rmse"]["dt_min"], 2, 3, sticky="EW", width=5))
        self.inputs.append(utility.create_entry(input_container, self.data["rmse"]["dt_max"], 2, 5, sticky="EW", width=5))
        
        utility.create_button(self.ui.container, "Plot Reference and Calculated Comparison", 0, 0, lambda: self.collect_outputs("reference_comparison"), pady=(5, 10), ipady=14, sticky="EW", fg="black", bg="pink")
        utility.create_button(self.ui.container, "Plot Linear RMS Error vs Number of Particles", 2, 0, lambda: self.collect_outputs("linear"), pady=(5, 0), ipady=14, sticky="EW", fg="black", bg="pink")
        utility.create_button(self.ui.container, "Plot Logarithmic RMS Error vs Number of Particles", 3, 0, lambda: self.collect_outputs("log"), ipady=14, sticky="EW", fg="black", bg="pink")
        utility.create_button(self.ui.container, "Back", 4, 0, self.ui.create_menu_buttons, pady=(5, 0), ipady=10, sticky="EW", fg="black", bg="pink")
    ...
    def collect_outputs(self, type: str):
        if type is "reference_comparison":
            ...
            for index, entry in enumerate(self.inputs):
                self.outputs[keys[index]] = utility.parse_entry(default_values[index], utility.get_entry(entry))
    ...
    def plot(self, type: str):
        if type is "reference_comparison":
            ...
            assert np.all(particles > 0), "Particle array for reference comparison cannot have numbers less than 1"
            assert dt <= self.data["parameters"]["time_max"], "Cannot use greater dt than time max for reference comparison"
            figure = self.validation.reference_comparison_figure(particles, dt, line_styles=['--', '-.', ':'])
        else:
            # Convert particle min and max to exponents for logspace domain to work properly.
            extents = np.log([self.outputs["particle_min"], self.outputs["particle_max"]]) / np.log([10])
            particles = np.logspace(extents[0], extents[1], self.outputs["particle_divisions"], dtype=int)
            
            dts = np.linspace(self.outputs["dt_min"], self.outputs["dt_max"], self.outputs["dt_divisions"])
        ...
        utility.create_button(self.ui.container, "Back", 1, 0, self.press, pady=(5, 0), ipady=15, fg="black", bg="pink")
    ...
    def output_validation(self, outputs: Dict[str, any]):
        ...
        valid = utility.check(outputs["particle_divisions"] > 0, "Number of particle divisions must be greater than 0") & \
                ...
                utility.check(outputs["dt_max"] <= simulation_max_time, "Maximum time step cannot exceed simulation maximum time (" + str(simulation_max_time) + ")")
        return valid
...
class CustomConditions(MainMenuButton):
    def press(self):
        ...
        utility.create_button(self.ui.container, "Run Simulation", row_count - 1, 0, self.collect_outputs, pady=(5, 0), ipady=10, fg="black", bg="pink")
        ...
        for input in self.inputs:
            ...
            if isinstance(keys, list):
                for index, entry in enumerate(input.entries):
                    self.outputs[keys[index]] = utility.parse_entry(defaults[index], utility.get_entry(entry))
            else:
                self.outputs[keys] = utility.parse_entry(defaults, utility.get_entry(input.entries))
        # Any value of None (invalid) in the outputs dictionary will prevent the button from continuing.
        if not utility.contains_value(self.outputs, None) and self.output_validation(self.outputs):
            self.plot()
    ...
    def plot(self):
        ...
        utility.create_button(self.ui.container, "Reset Plot", 1, 0, self.plot, pady=(5, 0), ipady=15, fg="black", bg="pink")
        utility.create_button(self.ui.container, "Back", 2, 0, self.press, pady=(5, 0), ipady=15, fg="black", bg="pink")
    ...
    def output_validation(self, outputs: Dict[str, any]):
        ...
        valid = utility.check(outputs["time_max"] > 0, "Max time must be greater than 0") & \
                ...
                utility.check(outputs["cell_size"][Y] > 0, "Cell height must be greater than 0")
        
        if outputs["use_velocity"]:
            # Check that the velocity field file can be read and has 4 columns.
            coordinates, vectors = utility.read_data_file(outputs["velocity_field_path"], [0, 1], [2, 3])
