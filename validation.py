...
class Validation(object):
    ...
    def __init__(self, sim_args: Dict[str, any]):
        ...
        self.reference_cells = np.linspace(
            self.sim_args["min"][0], self.sim_args["max"][1], self.sim_args["cell_size"][0])
        self.reference_concentrations = reference_function(
    ...
    def reference_comparison_figure(self, particles: npt.NDArray, dt: float, line_styles: List[str] = None):
        ...
        calculated_concentrations = self.__get_concentrations(
            particles, np.array([dt]))
        plt.plot(self.reference_cells,
                 self.reference_concentrations, label="Reference")
        ...
        for index, calculated_concentration in enumerate(calculated_concentrations[0]):
            plt.plot(self.reference_cells, calculated_concentration,
                     label="Number of Particles: " + str(particles[index]), linestyle=line_styles[index])
        utility.configure_plot(
            "Concentration ϕ vs x (t=0.2s, dt=" + str(dt) + "s)", "x", "Concentration ϕ", "linear")
        return figure
    ...
    def rmse_figure(self, particles: npt.NDArray, dts: npt.NDArray, rmse_array: npt.NDArray, scale: str, plot_curve_fits: bool = False,
        ...
        utility.configure_plot("RMS Error vs Number of Particles (" +
                               scale + ")", "Number of Particles", "RMS Error", scale)
        return figure
    ...
    def fit_rmse_curve(self, particles: npt.NDArray, dts: npt.NDArray):
        ...
        for index in range(len(dts)):
            ...
            filtered_rmse = lfilter(
                [1.0 / smoothing] * smoothing, 1, x=rmse_array[index])
            parameters, _ = curve_fit(
                E, particles, filtered_rmse, p0=[1, -0.5])
