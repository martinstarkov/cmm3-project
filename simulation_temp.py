...
class Simulation(object):
    ...
    def __enforce_boundary_conditions(self):
        ...
        self.coordinates[:] = np.where(self.coordinates < self.min, self.min + (self.min - self.coordinates), self.coordinates)
        self.coordinates[:] = np.where(self.coordinates > self.max, self.max + (self.max - self.coordinates), self.coordinates)
